...
class _CanvasScreenState extends State<CanvasScreen> {
      ...
      child: Row(children: [
        ...
        Container(
          ...
          child: Text(_dimState.scale.drawingScale, style: const TextStyle(
            color: Color(0xFFFFD93D), fontSize: 11,
        ...
        if (_isImporting || _isExporting || _isSaving)
          ...
          _TopBtn(Icons.zoom_out_map, 'Réinitialiser', _resetView),
          _TopBtn(Icons.grid_on, 'Grille',
              () => setState(() => _showGrid = !_showGrid),
              isActive: _showGrid),
          _TopBtn(Icons.layers, 'Calques',
              () => setState(() => _showLayers = !_showLayers),
              isActive: _showLayers),
          _TopBtn(Icons.save_outlined, 'Sauvegarder', _saveProject),
          const SizedBox(width: 4),
          _ExportMenu(onPng: _exportPng, onPdf: _exportPdf, onDxf: _exportDxf),
        ],
      ]),
    );
  }
  ...
  Widget _buildDimSidePanel() {
    return Container(
      ...
      child: Column(children: [
        ...
        _SideIcon(Icons.straighten, 'Cote',
            _dimState.activeTool == DimensionTool.linearDim,
            () => _dimState.setTool(DimensionTool.linearDim)),
        _SideIcon(Icons.call_made, 'Repère',
            _dimState.activeTool == DimensionTool.leaderLine,
            () => _dimState.setTool(DimensionTool.leaderLine)),
        const Divider(color: Color(0xFF2A2A2A), height: 20),
        _SideIcon(Icons.horizontal_rule, 'Guide H',
            _dimState.activeTool == DimensionTool.guideH,
            () => _dimState.setTool(DimensionTool.guideH)),
        _SideIcon(Icons.vertical_distribute, 'Guide V',
            _dimState.activeTool == DimensionTool.guideV,
            () => _dimState.setTool(DimensionTool.guideV)),
      ]),
    );
  }
  ...
  void _onScaleStart(ScaleStartDetails d) {
    ...
    if (d.pointerCount >= 2) {
      _isPanning = true; _state.endStroke(); return;
    }
    _isPanning = false;
  ...
  Future<void> _saveProject() async {
    ...
    try {
      await ProjectService.saveProject(
        state: _state, title: _projectTitle,
  ...
  Future<void> _exportPng() async {
    ...
    try {
      ...
      if (bytes != null) await ExportService.shareFile(bytes,
          ExportService.timestampedName('trace', 'png'), 'image/png');
    } finally {
  ...
  Future<void> _exportPdf() async {
    ...
    try {
      ...
      if (pdf != null) await ExportService.shareFile(pdf,
          ExportService.timestampedName('trace', 'pdf'), 'application/pdf');
    } finally {
      if (mounted) setState(() => _isExporting = false);
    }
  }
  ...
  Future<void> _exportDxf() async {
    ...
    try {
      ...
      await ExportService.shareFile(bytes,
...
class _ModeToggle extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(children: [
        _Tab('Dessin', Icons.edit_outlined,
            mode == CanvasMode.draw, () => onChanged(CanvasMode.draw)),
        _Tab('Symboles', Icons.chair_outlined,
            mode == CanvasMode.symbol, () => onChanged(CanvasMode.symbol)),
        _Tab('Cotes', Icons.straighten,
            mode == CanvasMode.dimension,
            () => onChanged(CanvasMode.dimension)),
      ]),
    );
  }
}
...
class _Tab extends StatelessWidget {
  ...
  const _Tab(this.label, this.icon, this.isActive, this.onTap);

  @override
  Widget build(BuildContext context) {
    final c = isActive
        ? const Color(0xFF4A9EFF) : const Color(0xFF555555);
...
class _SideIcon extends StatelessWidget {
  ...
  final bool isActive;
  final VoidCallback onTap;
  const _SideIcon(this.icon, this.label, this.isActive, this.onTap);
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: AnimatedContainer(
      ...
      child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        Icon(icon, size: 18,
            color: isActive
                ? const Color(0xFF4A9EFF) : const Color(0xFF666666)),
...
class _TopBtn extends StatelessWidget {
  ...
  final VoidCallback onTap;
  final bool isActive;
  const _TopBtn(this.icon, this.tooltip, this.onTap, {this.isActive = false});
    ...
    child: GestureDetector(
      ...
      child: Container(
        ...
        child: Icon(icon, size: 18,
            color: isActive
                ? const Color(0xFF4A9EFF) : const Color(0xFF888888)),
      ),
    ),
  );
}
...
class _ExportMenu extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return PopupMenuButton<String>(
      ...
      itemBuilder: (_) => [
        _item('png', Icons.image_outlined, 'PNG haute résolution',
            const Color(0xFF50E3A4)),
        _item('pdf', Icons.picture_as_pdf_outlined, 'PDF imprimable',
            const Color(0xFFFF6B35)),
        _item('dxf', Icons.architecture, 'DXF — AutoCAD',
  ...
  PopupMenuItem<String> _item(String val, IconData icon,
